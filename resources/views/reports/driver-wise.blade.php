<x-app-layout>
    ...
    <x-slot name="header">
        ...
        <div><h1 class="text-2xl font-bold text-slate-900">Driver-wise Report</h1><p class="mt-1 text-sm text-slate-600">Purchase totals attributed to each driver.</p></div>
    ...
    <x-reports.filter-card
        ...
        :export="route('reports.driver-wise.export', request()->query())"

        :pdf="route('reports.driver-wise.pdf', request()->query())"
    ...
    <x-reports.print-shell title="Driver-wise Purchase Report" :summary="'Total amount: '.number_format($totals['amount'], 2).' '.$company->currency">
        ...
        <tbody>
            ...
            @forelse ($rows as $row)
                ...
                <tr>
                    ...
                    <td class="col-primary font-medium" data-label="Driver">{{ $row['driver'] }}</td>
